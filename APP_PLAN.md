...
- 以 Xcode 26.6、Swift 6.3.3 编译器、Swift 6 语言模式、SwiftUI 构建，支持 iPhone 与 iPad（最低 iOS 26.5）；iPad 支持四方向与多任务窗口，录制成片保持固定竖屏画幅以确保画面和水印一致。
...
- 首次启动依次展示：温和的本地隐私介绍、成年与合法使用确认、私密空间密码设置、可选提示词、姓名与头像设置、保存方式选择。
  - 密码至少 12 个字符，区分大小写和空格；提示词最多 60 字，明确提示它不是密码且可能泄露线索。
...
- 录制前的固定说明为：
  ...
  > 我是【姓名】。我确认自己已经成年。我自愿与【对方姓名】发生性关系，未受到威胁、暴力、胁迫或不当压力。
